...
# 데이터베이스 초기화 (선택)
uv run python -c "from app.database import Base, engine; Base.metadata.create_all(bind=engine)"
...
# 테스트 실행
./test-run.sh           # 또는 uv run pytest tests/
...
| `./test-run.sh` 또는 `uv run pytest` | 모든 테스트 실행 |
| `uv run pytest tests/test_routers.py -v` | 특정 테스트 파일 실행 |
| `uv run black app/` | 코드 포맷팅 |
| `uv run mypy app/` | 타입 검사 |
| `uv run python scripts/ingest.py --file data/YYYY-MM-DD_report.html` | 데이터 적재 |
