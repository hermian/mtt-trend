...
from app.routers import themes, stocks, sync, charts, etf, heatmap, top30
...
async def lifespan(app: FastAPI):
    ...
    async def run_returns_prewarm():
        """서버 시작 시 주요 종목의 수익률 비교 데이터를 사전 웜업하여 사용자 첫 접속 시 0ms 체감 제공"""
        try:
            from app.utils.returns_utils import prewarm_returns_cache
            await asyncio.to_thread(prewarm_returns_cache)
            logger.info("Returns comparison startup pre-warm completed successfully.")
        except Exception as e:
    ...
    async def run_valuation_prewarm():
        """서버 시작 시 주요 지수의 Valuation 밴드 데이터를 사전 웜업하여 0ms 체감 제공"""
        try:
            from fastapi import Request
            from app.routers.charts import get_valuation_bands
            req = Request(scope={"type": "http", "headers": [(b"accept-encoding", b"gzip")]})
            for idx in ("kospi", "kosdaq"):
                for m in ("pbr", "per"):
                    await asyncio.to_thread(get_valuation_bands, req, index=idx, mode=m)
            logger.info("Valuation bands startup pre-warm completed successfully.")
        except Exception as e:
...
app.include_router(top30.router, prefix="/api")
