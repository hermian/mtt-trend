...
async def get_etf_heatmap(
    market: str = Query("KR", description="시장 구분 ('KR')"),
    ...
    """
    if market != "KR":
        raise HTTPException(status_code=400, detail="Only 'KR' market is supported currently.")
    try:
        data = load_etf_heatmap_data(date)
        return data
