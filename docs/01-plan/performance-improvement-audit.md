...
### ✅ AVWAP 수급 탭 — 시계열 생성 벡터화 + 페이로드 축소 (2026-09-13 22:15)

**측정 (`curl -4 http://127.0.0.1:8000`, 4회 최솟값)**

| 엔드포인트 | 웜(캐시 히트) | 크기 |
|---|---|---|
| `GET /api/charts/supply-demand?code=005930` | 16 ms | **4.05 MB** |
| `GET /api/charts/price-profile` | 1.5 ms | 33.9 KB |
| `GET /api/charts/period-sums` | 1.1 ms | 725 B |

웜은 이미 빠르다. 문제는 **콜드 370~430 ms**(005930·SK하이닉스·NAVER·GS 전부 동일 규모)이고,
`cProfile` 분해 결과 **`_build_series` 하나가 280 ms = 콜드 경로의 72%** 였다.

**원인** — `df.iterrows()` 가 행마다 pandas `Series` 를 새로 만든다. 5,526행이면 5,526개다.
그리고 payload 4.05 MB 의 **99.3% 가 `series`** 인데, 그 안의 `norm_accumulation` 은
**프론트에서 읽는 곳이 한 곳도 없었다**(전수 grep). 게다가 숫자가 float64 전체 정밀도
(`15.040100019398798`)로 나가 gzip 엔트로피를 키우고 있었다.

#### 적용한 변경 (P0~P3)

| 순위 | 조치 | 대상 |
|---|---|---|
| P0 | `_SUGEUB_CACHE` **상한 6** 추가 | `sugeub_utils.py` |
| P1 | `_build_series` **컬럼 tolist 벡터화** (iterrows 제거) | `sugeub_utils.py` |
| P2 | 시계열 숫자 **소수 2자리 반올림** | `sugeub_utils.py` |
| P3 | `norm_accumulation` **엔드투엔드 제거** | `sugeub_utils.py`, `schemas.py`, `lib/api.ts`, `constants.ts`, 테스트 |

- **P0 근거**: 엔트리 1개가 실측 **9.57 MB**, 키 공간 2,915 종목 → 상한이 없으면 **27.9 GB**.
  프로젝트의 다른 캐시 9종은 전부 상한이 있는데 여기만 없었다(유일한 무제한 캐시).
  `_SUGEUB_CACHE_MAX = 6` 이면 최악 57 MB — swap 16/17.4 GB 인 이 머신에서 감당 가능한 선.
- **P2 주의점**: `round(-0.0001, 2)` 는 `-0.0` 이고 JSON 에 `-0.0` 이 그대로 나간다.
  `_round_series` 에서 `0.0 if r == 0 else r` 로 정규화했다(테스트로 고정).
- **P3**: 죽은 코드 `NORM_COLS`, `_norm()`, `NORM_COLORS`, `CHART_HEIGHTS.norm` 도 함께 삭제.
  사용자가 "norm 차트 되살릴 계획 없음"을 확인해 준 뒤 진행했다.

#### 검증 — 개정 전 소스와의 동등성

`git show HEAD:backend/app/utils/sugeub_utils.py` 를 `importlib` 로 별도 모듈 로드해
양쪽 출력을 `json.dumps(sort_keys=True)` 로 deep 비교. **5/5 완전 동등, 키 순서까지 동일.**

| 종목 | 포인트 | 개정 전 | 개정 후 | 배수 | 동등성 |
|---|---|---|---|---|---|
| 005930 | 5,526 | 304.2 ms | **27.8 ms** | **10.9×** | ✅ |
| 000660 | 5,526 | 290.9 ms | **45.9 ms** | 6.3× | ✅ |
| 035420 | 5,526 | 301.0 ms | **27.9 ms** | 10.8× | ✅ |
| 078930 | 5,372 | 290.3 ms | **28.1 ms** | 10.3× | ✅ |
| 039490 | 5,444 | 286.2 ms | **27.1 ms** | 10.6× | ✅ |

**엔드포인트 최종 수치 (005930)**

| 지표 | 이전 | 이후 | 개선 |
|---|---|---|---|
| 콜드 계산 | 412.9 ms | **109.3 ms** | **3.8×** |
| 직렬화 크기 | 4.051 MB | **2.364 MB** | **−42%** |
| gzip6 시간 | 58.0 ms | **28.6 ms** | 2.0× |
| gzip6 크기 | 1.094 MB | **0.411 MB** | **−62%** |

- 캐시 상한: 18회 삽입 → 크기 6 유지, 최초 키 축출 확인.
- `pytest tests/ -q` → **239 passed** (237 + 신규 2건). 신규 테스트는
  `test_supply_demand_series_shape`(round2·`-0.0` 금지·키 집합)와
  `test_sugeub_cache_is_bounded`(상한·FIFO 축출).
- 프론트 `AvwapSugeubPanel.test.tsx` → **4 passed**.

#### 기각 (측정 근거)

| 대안 | 측정 | 판정 |
|---|---|---|
| `load_supply_demand_raw` 명시 컬럼 | 14.5 ms | 현행 15 ms — 무의미 |
| `fetchall()` + `DataFrame` 직접 구성 | 14.4 ms | 무의미 |
| `parse_dates` | 15.6 ms | 무의미 |
| DuckDB 공유 커넥션 | `connect` 5 ms vs 쿼리 25 ms | 이득 없음 |

앞서 이 함수를 73 ms 로 본 것은 **콜드 페이지 캐시**였고, 웜에서는 15 ms 다.
이미 충분히 빠른 경로를 건드릴 이유가 없었다.

#### 잔여 — P4 (미적용, 판단 대기)

프론트 기본 구간은 `DEFAULT_SUGEB_RANGE = "2y"` = **504 / 5,526 포인트(9.2%)** 인데
서버는 항상 전체 이력을 보낸다. 기본 구간만 보내면 raw 0.37 MB·gzip 4 ms 규모가 되지만
**클라이언트(구간 전환·캐시 키) 변경이 필요**해 별도 결정으로 남겼다.

또 하나: 캐시 히트여도 **gzip 28.6 ms 가 매 요청 발생**한다(사전 압축 캐시가 없음).
`wics-index/all` 처럼 압축 결과를 캐시하면 이론상 0 에 가까워지지만, P4 로 페이로드 자체를
줄이는 편이 더 근본적이다.
